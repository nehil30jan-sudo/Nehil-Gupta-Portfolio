...
def home_page():
    ...
    try:
        ...
        st.warning(f"Unable to find {PHOTO_FILENAME}. Please ensure it is uploaded to your GitHub repository.")
...
def projects_page():
    ...
    with st.expander("2. 🤝 Partnership Business Development Framework"):
        st.markdown("""
        <div class='project-card-content'>
            <p><strong>Objective:</strong> Enhance the PPL Partnerships program across priority markets.</p>
    ...
    with st.expander("3. 🏆 GDM Operations Hackathon: AI-Driven Automation (Winner)"):
        st.markdown("""
        <div class='project-card-content'>
            <p><strong>Objective:</strong> Eliminate operational bottlenecks in contact discovery for prospecting teams.</p>
    ...
    with st.expander("5. ⚙️ Disablement Menu Automation & Re-engineering"):
        st.markdown("""
        <div class='project-card-content'>
            <p><strong>Impact:</strong> Increased review-publishing efficiency by 3x (from 16% to 48%).</p>
        </div>
        """, unsafe_allow_html=True)
...
def experience_page():
    ...
    with st.expander("🚀 G2 | Senior Specialist (Operations) | Feb 2026 – Present", expanded=True):
        ...
        * **Executive Support:** Partner with Senior Leadership Team (SLT) on strategic narratives.
        """)
    ...
    with st.expander("📊 Gartner (Full Tenure: 4 yrs 7 mos)"):
        ...
        with st.popover("Senior Specialist (Operations) | Dec 2025 – Feb 2026"):
            st.write("* **Analytics:** Streamlined complex workstreams to drive revenue.")
        with st.popover("Product Reviews Specialist | Oct 2023 – Dec 2025"):
            st.write("* **Efficiency:** Led automation projects; Won **GDM Operations Hackathon 2025**.")
        with st.popover("Product Reviews Associate | Aug 2021 – Sep 2023"):
            st.write("* **Validation:** Verified reviews on Gartner platforms (Capterra, GetApp).")

    with st.expander("🎨 MRM//McCann, Freelance & Ureka"):
        st.write("* **MRM//McCann:** Managed digital marketing for retail accounts.")
        st.write("* **Freelance:** Prepared competitive matrices for **Fortune 1000 companies**.")
        st.write("* **Ureka Group:** Won **Global Youth Icon (2021)**.")
...
def education_page():
    ...
    st.markdown("<div class='edu-card'><strong>G.D. Goenka University</strong><br>BBA, Marketing | 2018 – 2021<br>⭐ <strong>CGPA: 8.95</strong> (Final Sem SGPA: 9.67)</div>", unsafe_allow_html=True)

    st.markdown("### Schooling")
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("<div class='edu-card'><strong>MM Public School, Rohini</strong><br>Class XII (CBSE) | 📚 PCM + PE & English<br>⭐ Result: <strong>90%</strong></div>", unsafe_allow_html=True)
    with col2:
...
def achievements_page():
    ...
    if ach_type == "Corporate Awards":
        ...
        st.warning("**YouTube Content Creation:** Monetized Gaming channel with **3,000+ subscribers**.")
...
pg = st.navigation([
    ...
    st.Page(contact_page, title="Contact Info", icon="📧"),
])
